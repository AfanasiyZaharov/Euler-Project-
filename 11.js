...
const mainFunction = () => {
  const badSectors = resultMatrix.reduce((accum, elem, index) => {
    let resultIndexesOfZeroes = elem.reduce((accum, elem, index) => {
      if (elem === "00") {
        return [...accum, index];
      } else {
        return accum;
    ...
    if (resultIndexesOfZeroes.length > 0) {
      return [...accum, ...resultIndexesOfZeroes.map((indexElem) => ({ y: index, x: indexElem }))];
    } else {
      return accum;
    }
  }, []);
